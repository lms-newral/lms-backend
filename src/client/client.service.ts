...
export class ClientService {
    ...
    async createClient(data: {
        ...
        if (!data.slug || typeof data.slug !== 'string' || !/^[a-z0-9-]+$/.test(data.slug)) {
            throw new Error('Slug is required and must be a lowercase alphanumeric string (dashes allowed).');
        }
        if (!data.subdomain || typeof data.subdomain !== 'string' || !/^[a-z0-9-]+$/.test(data.subdomain)) {
            throw new Error('Subdomain is required and must be a lowercase alphanumeric string (dashes allowed).');
        }
        ...
        if (existing) {
            throw new Error('A client with the same slug or subdomain already exists.');
        }
